...
MailboxView.prototype = {
	...
	reflectMessages: function(messages){
		...
		var groups = (function(){
			var out = {};
			messages.forEach(function(mo){
				var group_id = (function(){
					if(mo.mailbox.substring(0, 'SlateMail/scheduled/'.length) === 'SlateMail/scheduled/'){
						return 'Past Due';
					}
					return self.getDateString(mo.date);
				}());
				if(!out[group_id]){
					out[group_id] = [];
				}
				out[group_id].push(mo);
			});
			return out;
		}());
		console.log(groups);
